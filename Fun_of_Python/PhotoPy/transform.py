...
def brighten(image, factor):
    ...
    # Greater than x > 1 brightens, Less than 0 < x < 1 darken
    x_pixels, y_pixels, num_channels = image.array.shape  # represents x, y pixels of image, # channels (R, G, B)
    new_im = Image(x_pixels=x_pixels, y_pixels=y_pixels, num_channels=num_channels)  # making a new array to copy image
...
def adjust_contrast(image, factor, mid):
    # adjust the contrast by increasing the differences
    x_pixels, y_pixels, num_channels = image.array.shape  # represents x, y pixels of image, # channels (R, G, B)
...
def blur(image, kernel_size):
    ...
    # kernel size has to be odd number b/c ... 
    # minus the pixel its starting at the kernel size is the diameter and the radius the kernel -1 /2 in all directions
    x_pixels, y_pixels, num_channels = image.array.shape  # represents x, y pixels of image, # channels (R, G, B)
    new_im = Image(x_pixels=x_pixels, y_pixels=y_pixels, num_channels=num_channels)  # making a new array to copy values to!
    ...
    for x in range(x_pixels):
        for y in range(y_pixels):
            for c in range(num_channels):
                    #beginner method, larger kernel sizes that take longer to develop
                total = 0
...
if __name__ == '__main__':
    ...
    # Sobel edge highlights the edge
    sobel_x = apply_kernel(city, np.array([[1, 2, 1], [0, 0, 0], [-1, -2, -1]]))
    sobel_x.write_image('edge_x.png')
    sobel_y = apply_kernel(city, np.array([[1, 0, -1], [2, 0, -2], [1, 0, -1]]))
    sobel_y.write_image('edge_y.png')

    # Combine and Make an edge detector!
    sobel_xy = combine_images(sobel_x, sobel_y)
    sobel_xy.write_image('edge_xy.png')

# computer may crash if do all at once, so i commented them but feel free to try the others